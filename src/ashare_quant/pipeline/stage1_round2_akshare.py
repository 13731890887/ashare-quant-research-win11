...
from ashare_quant.data.akshare_loader import fetch_universe_daily, save_parquet, load_local_parquet
...
def run() -> None:
    ...
    try:
        raw, fetch_errors = fetch_universe_daily(symbols, start_date, end_date)
        save_parquet(raw, "data/raw_daily_round2.parquet")
    except Exception:
        raw = load_local_parquet("data/raw_daily_round2.parquet")
    ...
    if "data_vendor" in raw.columns:
        metrics["vendors"] = sorted(raw["data_vendor"].dropna().unique().tolist())

    log_experiment(
        "stage1_round2_akshare_baseline",
        {
            "symbols": len(symbols),
            "top_n": 8,
            "hold_days": 5,
